...
-(void) configureCastPlaybackViewBasedOnItem:(OOVideo*)item displayName:(NSString*)displayName displayStatus:(NSString*)displayStatus videoTitle:(NSString*)videoTitle videoImage:(NSString*)videoImage {
    if( self.textView == nil ) {
        [self buildPlaybackView:item];
    }
    [self updateTextView:item displayName:displayName displayStatus:displayStatus videoTitle:videoTitle videoImage:videoImage];
}
...
-(void) buildPlaybackView:(OOVideo*)item {
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        dispatch_sync(dispatch_get_main_queue(), ^{
            ...
            [self.textView setFont:[UIFont boldSystemFontOfSize:26]];
...
-(void) updateTextView:(OOVideo*)item displayName:(NSString*)displayName displayStatus:(NSString*)displayStatus videoTitle:(NSString*)videoTitle videoImage:(NSString*)videoImage {
    if( self.textView ) {
        dispatch_async( dispatch_get_main_queue(), ^{
            //NSString *videoDescription = item.itemDescription;
            
            NSData * imageData = [[NSData alloc] initWithContentsOfURL: [NSURL URLWithString: videoImage]];
